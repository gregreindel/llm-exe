 ...
 *
 * This function:
 * 1. Checks if messages have metadata indicating they came from conversion
 * 2. If so, uses the converter to reconstruct the original OpenAI format
 * 3. Otherwise, falls back to the legacy transformation logic
 */
export function promptSanitize(messages: any) {
  ...
  if (hasMetadata) {
    try {
      // Messages have metadata - use converter to reconstruct OpenAI format
      const openAIMessages = fromInternal(
      ...
      console.warn("Failed to convert messages back to OpenAI format:", error);
