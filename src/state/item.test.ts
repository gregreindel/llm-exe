...
describe("llm-exe:state/BaseStateItem", () => {
    ...
    it("item.getKey", () => {
      ...
      it("item.setValue allows setting a value when default is undefined", () => {
      ...
      it("item.setValue skips type check after resetValue when default was undefined", () => {
        const item = new MockStateItem<string | undefined>("intent", undefined);
        item.setValue("hello");
        item.resetValue();
        expect(item.getValue()).toBeUndefined();
        item.setValue("world");
        expect(item.getValue()).toEqual("world");
      });
})