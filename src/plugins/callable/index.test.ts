...
describe("llm-exe:callable/CallableExecutor", () => {
  ...
  it("CallableExecutor from async", () => {
    ...
    expect(callableFn1.input).toEqual(
      `Must be JSON: ${JSON.stringify({ accountId: "12345" })}`
    );
  ...
  it("CallableExecutor from llm", async () => {
    ...
    const res = await callableFn1.execute({});
    expect(res).toEqual({
      result: `Hello world from LLM! The input was [{\"role\":\"system\",\"content\":[{\"type\":\"text\",\"text\":\"This is a prompt.\"}],\"_meta\":{\"original\":{\"provider\":\"openai\",\"wasArray\":false}}}]`,
      attributes: {},
    });
  });
});
