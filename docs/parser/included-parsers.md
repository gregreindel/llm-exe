...
`markdownCodeBlock`
Extracts the **first** code block from the LLM response, including the language identifier. If no code block is found, returns `{ code: "", language: "" }`. To extract all code blocks, use [`markdownCodeBlocks`](#markdown-code-blocks) (plural) instead.
...
`markdownCodeBlocks`
Extracts **all** code blocks from the LLM response, returning them as an array. Use this when the response may contain multiple code blocks. For extracting only the first code block, use [`markdownCodeBlock`](#markdown-code-block) (singular) instead.
