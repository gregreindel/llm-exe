import { configs } from '../../../src/llm/config';
...
export function generateProviders(): ProviderConfig[] {
  const providerMap = new Map<string, ProviderConfig>();

  Object.entries(configs).forEach(([configKey, config]: [string, any]) => {
    let providerKey = configKey.split(/[.:]/)[0];
    let providerName = getProviderDisplayName(providerKey);
    ...
    if (!model && config && typeof config === "object") {
      if (
        config.options &&
        config.options.model &&
        config.options.model.default
      ) {
        model = config.options.model.default;
      }
    }

    if (model === "chat" && providerKey === "deepseek") {
      model = "deepseek-chat";
    }
    ...
    if (model) {
      if (!providerMap.has(providerKey)) {
        providerMap.set(providerKey, {
          key: providerKey,
          name: providerName,
          logo: providerLogos[providerKey] || providerLogos.openai,
          models: [],
        });
      }
      providerMap.get(providerKey)!.models.push(model);
    }
  });

  if (providerMap.has("bedrock")) {
    const bedrockProvider = providerMap.get("bedrock")!;
  ...
  providers.forEach((provider) => {
    provider.models = [...new Set(provider.models)].sort();
  });
